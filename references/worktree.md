...
2. The project's `WorktreeRemove` hook when its settings have a `hooks.WorktreeRemove` entry: the script it names, `${CLAUDE_PROJECT_DIR}` read as `<home>`, fed what Claude Code would send it: `cd <home> && echo '{"hook_event_name":"WorktreeRemove","worktree_path":"<path>"}' | .claude/hooks/worktree-remove.sh` (the `cd` is what makes it cwd-proof: the script resolves the repo from where it runs). The script is not in `<home>` (hook files reach a tree only once committed): say so, the tool it undoes stays linked, and `git -C <home> worktree remove <path>` instead. No entry: the same git command.
