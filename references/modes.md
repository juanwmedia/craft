...
How much the human is in the loop. Inherited by every skill (`/shape`, `/spec`, `/build`, `/close`, `/evaluate`). **Default: `in the loop`.** Modes switch only when the human says so, never silently.
