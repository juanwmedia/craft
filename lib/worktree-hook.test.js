...
test('remove hook: a Herd link of the same name that points somewhere else is left alone', () => {
  const dir = repo(true);
  const wt = create(dir, 'x');
  ...
  fs.symlinkSync(someoneElse, path.join(sites, 'x'));

  const bin = fs.mkdtempSync(path.join(os.tmpdir(), 'craft-bin-'));
  const marker = path.join(bin, 'unlinked.txt');
  fs.writeFileSync(path.join(bin, 'herd'), `#!/bin/bash\necho "$@" >> ${marker}\n`, { mode: 0o755 });

  execFileSync('bash', [path.join(LIB, 'worktree-remove.sh')], {
    cwd: dir, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'],
    input: JSON.stringify({ session_id: 's', transcript_path: '/tmp/t.jsonl', cwd: dir, hook_event_name: 'WorktreeRemove', worktree_path: wt }),
    env: { ...process.env, HOME: home, PATH: bin + ':' + process.env.PATH },
  });

  assert.ok(!fs.existsSync(marker), 'herd unlink must not run for a link pointing elsewhere');
  assert.ok(fs.existsSync(path.join(sites, 'x')), 'the foreign link must survive');
