...
test('herd, remove: a link of the same name that points somewhere else is left alone', () => {
  const dir = repo(true);
  const wt = create(dir, 'x');
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'craft-home-'));
  const someoneElse = fs.mkdtempSync(path.join(os.tmpdir(), 'craft-site-'));
  fs.symlinkSync(someoneElse, path.join(herdSites(home), 'x'));
  ...
  assert.ok(fs.existsSync(path.join(home, 'Library', 'Application Support', 'Herd', 'config', 'valet', 'Sites', 'x')), 'the foreign link must survive');
...
test('herd, create then remove: the tree is linked by name, and unlinked when worktree_path arrives through a symlink to a link stored resolved', () => {
  const dir = repo(true);
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'craft-home-'));
  const { bin, marker } = herdStub();
  const wt = create(dir, 'x', bin, withTool('worktree-create.sh', 'herd-create', 'echo "$path"'), home);
  assert.strictEqual(fs.readFileSync(marker, 'utf8'), 'link x\n');
  fs.symlinkSync(fs.realpathSync(wt), path.join(herdSites(home), 'x'));
