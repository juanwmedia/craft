...
- **Traceability** — every task maps to acceptance criteria. Every decision records its rationale.
...
4. **Create native tasks** for tracking.
5. **Execute** — spawns subagents for parallel batches, keeps sequential what needs control. Prioritizes correctness over speed.
6. **If something fails** — re-enters plan mode, adjusts, appends to the iteration log. Repeats until stable.
...
- Spec gaps are noted for `/close` — the spec is NOT updated during build.
...
Reconciles what was built against what was specified. Updates artifacts, captures learnings, proposes commits.
...
6. **Propose commits** — atomic, WHY-focused. User must approve.
...
2. /build user-auth
   ...
   → Executes: subagents for parallel batches, sequential for dependencies
...
3. /close user-auth
   ...
   → Captures cross-cutting decisions
...
- `spec.md` has `spec_version` in frontmatter. Incremented when acceptance criteria change (during `/close`).
...
4. **Traceability is mandatory.** Every task maps to acceptance criteria. Every tech-plan links to the spec version it was designed against. Every decision records its rationale.
