...
4. `/craft:close <slug>` reconciles `shape.md` and `slices.md` with what was actually built, because they will have drifted: the files each slice touched, the assumptions that held or broke, the drawings that no longer match. It writes the few lessons the code cannot tell into the repo's `CLAUDE.md` or `AGENTS.md`, then asks you once before it commits, pushes and, if you want, opens the pull request. It is the only step that commits or pushes, so run it every time you want to keep what changed.
