...
4. **Execute.** Task by task, flipping `status`. A plan that turns out wrong is a `friction` with `relatesTo`. Writing is never delegated: subagents are read-only recon or `review`. A file outside the plan is scope drift, logged.
