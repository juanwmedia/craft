...
**`above the loop`.** Hand the phase to the `craft:delegate` agent (`Agent`): the board is its brief, plus one line from the human on what it must not touch, the boundary it cannot infer. It runs this very skill, every ⏸ a line of its one report, the phase boundary included, and proposes a commit. Sonnet by default, Opus when the human says so. The human reads the report; nothing else reaches them. If you are delegate, this paragraph is not for you: build the phase as `in the loop` says, with the substitutions your own brief gives you.
...
2. `craft:evaluate` (`Agent`) on the phase's coverage claims: the board's path and the phase's tasks and ACs.
